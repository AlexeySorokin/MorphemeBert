...
def load_merge_table(path, normalize=False):
    """
    ...
    with open(path) as f:
        for line in f:
            ...
            merge_table[(token_1, token_2)] = int(priority)
    if normalize:
        max_value = max(merge_table.values())
        for key, value in merge_table.items():
            merge_table[key] /= max_value
    return merge_table


def tokenize_word(merge_rules, word,
                  dropout=0.0, noise=0.0,
    ...
    if noise_generator is None:
        noise_generator = random_generator
    sw_tokens = list(word)
    ...
    for pos in range(len(sw_tokens) - 1):
        ...
        if cur_nxt_pair in merge_rules:
            ...
            if noise > 0.0:
                cur_priority += noise * noise_generator.uniform()
    ...
    while len(merge_heap):
        ...
        coin = random_generator.rand()

        if verbose:
            print("Tokens:", sw_tokens)
            print(f"Iteration {iteration}, merge:{cur}+{nxt}->{cur}{nxt}, coin:{coin:.2f}")

        if (cur, nxt) not in merge_rules:
            continue
        ...
        if cur_pos > 0:
            ...
            if (prev, new_cur) in merge_rules:
                priority = merge_rules[(prev, new_cur)]
                if noise:
                    priority += noise * noise_generator.uniform()
                heapq.heappush(merge_heap, [priority, cur_pos - 1])

        if cur_pos < (sw_length - 1):
            new_next = sw_tokens[cur_pos + 1]
            if (new_cur, new_next) in merge_rules:
                priority = merge_rules[(new_cur, new_next)]
                if noise:
                    priority += noise * noise_generator.uniform()
                heapq.heappush(merge_heap, [priority, cur_pos])
